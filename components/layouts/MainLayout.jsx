...
import {
  ...
  Search, Plus, Menu, X, User, LogOut, Edit3, Check, Building,
  CreditCard, Cpu
} from 'lucide-react';
...
const getAdminName = () => {
  if (typeof window !== 'undefined') {
    return localStorage.getItem(ADMIN_KEY) || 'Super Admin';
  }
  return 'Super Admin';
};
const saveAdminName = (n) => {
  if (typeof window !== 'undefined') {
    localStorage.setItem(ADMIN_KEY, n);
  }
};

const NAV_ITEMS = [
  { icon: LayoutDashboard, label: 'Dashboard',          id: 'dashboard' },
  { icon: Building2,       label: 'Client Companies',   id: 'companies' },
  { icon: BarChart3,       label: 'Revenue & Billing',  id: 'billing' },
  { icon: Users,           label: 'Employee Analytics', id: 'analytics' },
  { icon: Ticket,          label: 'Support Tickets',    id: 'support' },
  { icon: ShieldCheck,     label: 'Feature Management', id: 'features' },
  { icon: Activity,        label: 'System Monitoring',  id: 'monitoring' },
  { icon: Lock,            label: 'Roles & Permissions',id: 'permissions' },
  { icon: Bell,            label: 'Notifications',      id: 'notifications' },
  { icon: History,         label: 'Audit Logs',         id: 'logs' },
  { icon: Cpu,             label: 'Integrations Hub',   id: 'hub' },
        ...
        {loading ? <div className="p-5 space-y-3">{[...Array(4)].map((_,i)=><div key={i} className="h-12 bg-gray-100/50 rounded-xl animate-pulse"/>)}</div> :
         ...
         items.map(n=>(
          <div key={n.id} className="flex items-start gap-3 px-5 py-3.5 hover:bg-white/50 transition-colors cursor-pointer group">
            <span className={`mt-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0 h-fit ${typeC[n.type]||'bg-gray-100 text-gray-600'}`}>{(n.type||'').toUpperCase()}</span>
...
function GlobalSearch({ setActiveTab }) {
  ...
  const handleSearchResultClick = (r) => {
    if (r.to.startsWith('/companies')) {
      setActiveTab('companies');
    } else if (r.to.startsWith('/billing')) {
      setActiveTab('billing');
    } else if (r.to.startsWith('/support')) {
      ...
      {open && results.length>0 && (
        <div className="absolute top-[calc(100%+8px)] left-0 right-0 glass-card rounded-2xl z-50 overflow-hidden shadow-2xl">
          <div className="py-2 max-h-80 overflow-y-auto">
            {results.map(r=>(
              <button key={r.type+r.id} onClick={() => handleSearchResultClick(r)} className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-[#4c58fa]/5 text-left transition-colors">
...
export default function MainLayout({ activeTab, setActiveTab, children }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-inter text-[#1e293b] selection:bg-[#4c58fa]/10 overflow-x-hidden flex flex-col">
      
      <div className="relative z-10 flex h-screen overflow-hidden flex-1">
        
        {/* Sidebar */}
        <aside className={cn('glass-sidebar flex flex-col z-40 transition-all duration-300 ease-in-out shrink-0',
          collapsed?'w-[80px]':'w-[280px]', mobileOpen?'translate-x-0':'-translate-x-full lg:translate-x-0 lg:static fixed top-0 bottom-0 left-0')}>
          ...
          <nav className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-1.5 custom-scrollbar">
            ...
            {NAV_ITEMS.filter(item => {
              if (item.id === 'hub') {
                return typeof window !== 'undefined' && window.location.pathname === '/hub';
              }
              return true;
              ...
              return (
                <button key={id} onClick={()=>{
                  ...
                  "group flex items-center gap-3 rounded-xl transition-all duration-200 w-full text-left",
          ...
          <div className="p-4 mt-auto">
            <button onClick={()=>setProfileOpen(true)} className={cn(
              "flex items-center gap-3 rounded-2xl hover:bg-white transition-all cursor-pointer border border-transparent hover:border-gray-200 w-full text-left",
              collapsed ? "h-12 w-12 justify-center mx-auto" : "p-3 w-full"
            )}>
              <div className="w-9 h-9 rounded-full bg-[#4c58fa] flex items-center justify-center text-white font-bold text-sm shadow-md shrink-0">{adminName[0]}</div>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          {/* Top Navbar */}
          <header className="glass-navbar flex h-20 items-center justify-between px-8 shrink-0">
