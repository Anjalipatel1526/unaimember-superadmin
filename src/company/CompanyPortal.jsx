...
import CompanyAttendance from './views/CompanyAttendance';
import CompanyPayroll from './views/CompanyPayroll';
...
export default function CompanyPortal() {
        ...
        <div className="lg:w-1/2 bg-gradient-to-br from-[#4c58fa] to-[#3d45e8] text-white p-12 lg:p-24 flex flex-col justify-between relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10 max-w-md my-12 lg:my-0">
            <h1 className="text-4xl lg:text-5xl font-extrabold leading-tight tracking-tight">
              Manage your workforce with simplicity
            </h1>
            <p className="text-blue-100/80 mt-6 text-sm lg:text-base leading-relaxed">
              Login to access your centralized company dashboard. Oversee employees, manage payroll schedules, track attendance, and chat with technical support.
            </p>
  ...
  const isPayrollEnabled = companyDetails?.payroll_enabled;

  const menuItems = [
    { id: 'overview', label: 'Overview', icon: Building2 },
    { id: 'employees', label: 'Staff Directory', icon: Users },
    { id: 'attendance', label: 'Attendance Board', icon: CalendarDays },
    ...(isPayrollEnabled ? [{ id: 'payroll', label: 'Payroll Manager', icon: CreditCard }] : []),
      ...
      <div className="relative z-10 flex h-screen overflow-hidden">
        ...
        <aside className={`glass-sidebar flex flex-col z-40 transition-all duration-300 ease-in-out w-[280px] ${
            ...
            <div className="flex items-center gap-3 min-w-0">
              {session.companyDetails?.logo_url ? (
                ...
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#4c58fa] shadow-lg shadow-[#4c58fa]/20 shrink-0">
                  <span className="text-white font-bold text-xl">{session.companyName?.[0] || 'C'}</span>
                </div>
              )}
          ...
          <nav className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-1.5 custom-scrollbar">
            <p className="px-4 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-2">Company Console</p>
        ...
        <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-gray-50/30">
            <div className="mx-auto max-w-7xl relative">
              {activeTab === 'overview' && (
                <CompanyOverview companyId={session.companyId} companyDetails={companyDetails} />
              )}
              {activeTab === 'employees' && (
                <CompanyEmployees companyId={session.companyId} />
              )}
              {activeTab === 'attendance' && (
                <CompanyAttendance companyId={session.companyId} />
              )}
              {activeTab === 'payroll' && isPayrollEnabled && (
                <CompanyPayroll companyId={session.companyId} companyDetails={companyDetails} />
              )}
              {activeTab === 'support' && (
                <CompanySupport companyId={session.companyId} companyDetails={companyDetails} />
              )}
              {activeTab === 'settings' && (
                <CompanySettings 
                  companyId={session.companyId} 
                  companyDetails={companyDetails} 
                  onUpdateCompany={updateCompanyDetails} 
                />
              )}
            </div>
          </div>
