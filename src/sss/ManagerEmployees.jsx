...
const BLANK_EMP = {
  ...
  department: '', designation: 'Employee', employment_type: 'Full-Time',
  status: 'Active', date_of_joining: '', date_of_birth: '',
  salary: '', address: '',
};

export default function ManagerEmployees({ companyId, onRefreshData }) {
  ...
  const filtered = employees.filter(e => {
    const isHr = (e.designation || '').toLowerCase().includes('hr');
    const isManager = (e.designation || '').toLowerCase().includes('manager');
    if (isHr || isManager) return false;
    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage and update records for your organization's staff members.</p>
        </div>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-white">
          <div className="relative flex-1 max-w-sm">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search employees..." className="input pl-9 h-10"/>
          </div>
        </div>
                  ...
                  <tr key={emp.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <p className="text-sm font-semibold text-gray-800">{emp.designation || 'Employee'}</p>
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Field label="Designation / Title">
              <input className="input bg-gray-55 cursor-not-allowed" value="Employee" disabled />
            </Field>
      ...
      <ConfirmModal 
        ...
        message="Are you sure you want to delete this employee? All associated files and data will be permanently purged."
